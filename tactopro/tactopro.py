...
class TactoPro:
    ...
    def sample_frames_manually(self, poses: List[np.ndarray]) -> List[TactoFrame]:
        ...
        return [TactoFrame(rgbframe=point, heightmap=None, contactmask=None, campose=pose, gelpose=None) for point, pose in zip(sampled_points, poses)]
    ...
    def save(self, frames: List[TactoFrame], save_path: str, headless: bool = False):
        ...
        with open(pose_path, "wb") as f:
            pickle.dump({"camposes": [frame.campose for frame in frames], "gelposes": [frame.gelpose for frame in frames]}, f)

        for i, frame in enumerate(frames):
            cv2.imwrite(osp.join(rgbframe_path, f"{i}.png"), Image.fromarray(frame.rgbframe.astype(np.uint8), "RGB"))
            cv2.imwrite(osp.join(heightmap_path, f"{i}.png"), frame.heightmap.astype(np.float32))
            cv2.imwrite(osp.join(contactmask_path, f"{i}.png"), 255 * frame.contactmask.astype(np.uint8))
