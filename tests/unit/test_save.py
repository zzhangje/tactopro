...
def test_save_data():
    ...
    assert osp.exists(
        "tests/tmp/digit/rgbframes"
    ), "RGB frames directory should be created."
    assert osp.exists(
        "tests/tmp/digit/heightmaps"
    ), "Height maps directory should be created."
    assert osp.exists(
        "tests/tmp/digit/contactmasks"
    ), "Contact masks directory should be created."
    ...
    assert osp.exists(
        "tests/tmp/digit_1/rgbframes"
    ), "RGB frames directory should be created."
    assert osp.exists(
        "tests/tmp/digit_1/heightmaps"
    ), "Height maps directory should be created."
    assert osp.exists(
        "tests/tmp/digit_1/contactmasks"
    ), "Contact masks directory should be created."
    ...
    assert osp.exists(
        "tests/tmp/digit_2/rgbframes"
    ), "RGB frames directory should be created."
    assert osp.exists(
        "tests/tmp/digit_2/heightmaps"
    ), "Height maps directory should be created."
    assert osp.exists(
        "tests/tmp/digit_2/contactmasks"
    ), "Contact masks directory should be created."
